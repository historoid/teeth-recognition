...
for path in ds.img_all_list[:heat_num]:
    ...
    for layer in [n for n in layer_names if 'conv' in n]:
        ...
        dst1 = str(layer) + '_' + str(fn_core) + '-map.png'
        cv2.imwrite('./re_test_heatmap' + dst1, heatmap)
        # ヒートマップを元画像に適応
        heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
        # 透明度n%でスーパーインポーズ
        superimposed_heatimg = heatmap * 0.6 + img_cv
        dst1 = str(layer) + '_' + str(fn_core) + '-impose.png'
        cv2.imwrite('./re_test_heatmap/' + dst, superimposed_heatimg)
# -
